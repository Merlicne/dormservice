...
public class DormController {
    ...
    @GetMapping("/dorm")
    public ResponseBody<List<DormModel>> getDormInfo(@RequestHeader("Authorization") String token) {
        token = token.substring(7);
        JwtToken jwtToken = JwtToken.builder().token(token).build();

        List<DormModel> dorm = dormService.getAllDorm(jwtToken);
    ...
    public ResponseBody<DormModel> getDormById(
        ...
        DormModel dorm = dormService.getDormById(id, jwtToken);
        return new ResponseBody<>(
                        200, 
                        "Success", 
                        dorm);
